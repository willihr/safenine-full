...
const UserGraphs: React.FC<UserGraphsProps> = ({
  ...
  useEffect(() => {
    const tempPacientMeasures = measures
    ...
    if (pacientInfoLoaded === false) {
      setPacientInfoLoaded(true);
      if (tempPacientMeasures.length > 1) {
        const dataToSet: ZoomDomainType = {
          x: [
            tempPacientMeasures[0].key,
            tempPacientMeasures[tempPacientMeasures.length - 1].key,
          ],
          y: undefined,
        };
